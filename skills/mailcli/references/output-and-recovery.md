...
A definite `messages.get` `not_found` means the read found no message for that ref. No effect occurred, but retrying the same ref cannot make progress. Follow `retryability:user_input_required`, `replay_allowed:false`, and `recovery.action:correct` by obtaining a fresh valid ref before another lookup. Transient read failures retain their safe retry guidance.

`smtp_data_incomplete` proves SMTP failed before its end-of-data terminator was attempted; the claim is cleared and an explicit send retry is safe. `imap_append_incomplete` proves APPEND failed before its terminating CRLF; the session is discarded. During `drafts send`, SMTP has already accepted the message, so the retained result is mirror-pending and recovery must reconcile without resubmitting SMTP; during `drafts reconcile`, Sent is searched before the known-incomplete APPEND is retried. For `smtp_submission_unknown`, `sent_mirror_pending`, `imap_append_outcome_unknown`, `imap_copy_outcome_unknown`, and `imap_move_outcome_unknown`, retain the operation ID and follow the emitted recovery command. A send attempt points to `drafts reconcile --ref DRAFT_REF --json`; a retained visible-compose attempt points to `drafts handoff-reconcile --ref DRAFT_REF --attempt ATTEMPT_ID --outcome opened|failed --confirm --json`; partial hydration points to `messages get --ref MESSAGE_REF --json` or `drafts open --message MESSAGE_REF --json`. Never blindly retry an outcome-unknown operation.
