...
Use MailCLI as the single boundary to accounts already configured in macOS Mail. MailCLI uses SMTP and IMAP under the hood for sending and mailbox mutations, with credentials stored in the macOS Keychain; never ask the user to paste account passwords, app-specific passwords, OAuth tokens, or any other credentials into chat. Direct the user to `mailcli send setup` (see Send below) for one-time credential storage. Reads reuse the authentication Mail.app already has.
