...
- Use `batch` for supported operations on explicit refs, with unique item IDs and published limits. Inspect every item; a failed or uncertain item never authorizes replay of successful siblings. A `delete` batch requires `--confirm`. Read the output guide before batching.
