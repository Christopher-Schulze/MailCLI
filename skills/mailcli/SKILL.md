...
- Use `batch` for supported operations on explicit refs, with unique item IDs and published input limits. Inspect `ok` and every item even when exit status is 0; a failed or uncertain item never authorizes replay of successful siblings. A `delete` batch requires `--confirm`. Read the output guide before batching.
...
Exit 0 normally indicates success, 1 operation failure, and 2 usage failure. A partial JSON batch can currently return exit 0 with `ok:false` and `batch_partial`; inspect its envelope and items. Only `sync --check --require-complete` may return exit 3 with `ok:true` for valid incomplete coverage. Read the output guide for teardown failures and retained partial evidence.
