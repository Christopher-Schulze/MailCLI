...
Drafts are private JSON files under `~/Library/Application Support/MailCLI/drafts`, not fragile unsaved Mail compose objects. The normal flow is `drafts create` -> `drafts inspect` -> optional `drafts update` -> `drafts send --confirm`. Replies and forwards require a store-bound source reference. At send time MailCLI revalidates that source against the local store; forwards additionally fingerprint original attachments from local MIME. Mail then creates the native compose without a redundant RFC-source Apple Event. MailCLI first captures a stable Mail-owned body, including the configured automatic signature for a new message or quoted content for a reply or forward, and then places the reviewed plain-text body before that exact native material. The hidden compose object must remain stable before and after MailCLI adds deduplicated reviewed recipients and private attachment snapshots. Mail 16's `null` value for an empty rich-text attachment collection is normalized to an empty list; an actual attachment getter failure remains an error. A missing body, changed signature or quote, unobserved body suffix, duplicate recipient, or missing attachment blocks send and requires verified removal of the unsent object.
...
`drafts save --ref` explicitly exports a local review draft to Mail's Drafts mailbox. It requires an exact configured `from` identity and refuses with `compose_busy` if Mail has a visible user-owned compose window, so that window is never closed or mixed into the operation. Hidden or retained backend objects do not block MailCLI's separately owned compose. MailCLI captures a Drafts-store baseline, closes its hidden compose object with Mail's standard `saving: yes` command, and verifies one unique resulting message from the store. Preparation failure or cancellation requests closure of the owned object with `saving: no` and verifies its disappearance; a retained backend returns `compose_cleanup_failed`. If Mail accepted the save but no unique result appears, it returns `draft_outcome_unknown` and retains the local draft; agents must not retry automatically. Gmail may rotate temporary identifiers during synchronization; `drafts open` can recover only within the already resolved Drafts mailbox using the bound subject and rejects ambiguity.
...
Plain text is the guaranteed outbound format because it is portable, readable, inspectable before sending, and fully represented by Mail's scripting interface. New messages place the supplied body before Mail's stable automatic signature; native replies and forwards place it before Mail's generated quoted content. Final read-back requires that exact reviewed-plus-native materialization and rejects arbitrary additional text. Outbound attachments use Mail's documented rich-text attachment insertion. Arbitrary HTML is unsupported: Mail 16 declares its scripting `html content` property deprecated and ineffective.
