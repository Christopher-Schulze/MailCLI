...
func TestMarkMessagePreservesActualFlagResults(t *testing.T) {
	tests := []struct {
		...
		{name: "partial junk transition retains actual state", result: flagBoundaryResult{evidence: transport.MutationEvidence{Outcome: transport.MutationOutcomePartial, FlagsState: transport.FlagObservationObserved, ActualFlags: []string{"\\Seen"}, FlagsSource: "FETCH"}, code: transport.CodeIMAPFlagsPartial}, wantCode: transport.CodeIMAPFlagsPartial, wantRead: true, wantState: transport.FlagObservationObserved},
		{name: "unsupported transition retains original classification", result: flagBoundaryResult{evidence: transport.MutationEvidence{Outcome: transport.MutationOutcomeNotStarted, FlagsState: transport.FlagObservationObserved, ActualFlags: []string{"$NotJunk"}, FlagsSource: "FETCH"}, code: transport.CodeIMAPFlagsUnsupported}, wantCode: transport.CodeIMAPFlagsUnsupported, wantState: transport.FlagObservationObserved},
...
func TestBatchMarkRetainsMissingAndConflictingEvidence(t *testing.T) {
	for _, test := range []struct {
		...
		{"partial", transport.CodeIMAPFlagsPartial, transport.MutationOutcomePartial, mail.BatchItemFailed, transport.FlagObservationObserved},
		{"unsupported", transport.CodeIMAPFlagsUnsupported, transport.MutationOutcomeNotStarted, mail.BatchItemFailed, transport.FlagObservationObserved},
	} {
		t.Run(test.name, func(t *testing.T) {
