...
The CLI normally adds a cross-process mutation lock. If its configuration directory or lock-client setup fails, it currently falls back to process-local ordering; `MAILCLI_IMAP_MUTATION_LOCK=off` also disables the cross-process lock. Serialize concurrent invocations externally when lock setup cannot be relied on.
